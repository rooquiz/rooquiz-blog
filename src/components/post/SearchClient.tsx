...
/**
 * 站内搜索。索引由 pnpm sync 在构建期投影成 public/search-index.json，
 * 页面加载后一次性拉下来在浏览器里建索引。
 *
 * 为什么不走 Postgres 全文检索：纯静态站为了搜索单开一个 serverless 端点不划算。
...
export function SearchClient() {
  ...
  const engine = useMemo(() => {
    ...
    const mini = new MiniSearch<SearchDoc>({
      ...
      // 按词切。连字符、em dash、斜杠都当分隔符，好让 "quiz-design" 能被 "design" 命中
      ...
      {hasQuery &&
        (results.length === 0 ? (
          ...
          <ul className="rows">
            {results.map(result => (
              <li key={result.id}>
                <Link href={sitePath(result.id)} className="row__link">
                  <p className="u-meta">
                    <time dateTime={result.publishedAt as string}>{formatDate(result.publishedAt as string)}</time>
                  </p>
