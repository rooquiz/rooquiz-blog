...
| 归属                               | 内容                                                                                                          |
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| `components/motion/HomeMotion.tsx` | 首页入场：云分层升起 → 袋鼠落座 → **触地那一帧**白丘被压出去一圈 → 常驻呼吸；外加袋鼠的指针侧倾与点一下弹一跳 |
...
**动的只有天上那些装饰，正文一个字都不动。** 云是「升起」不是「淡入」：最前那层
是纸色的，天与纸的交界全靠它盖住，一透明就露出 `.sky` 那条矩形底边 —— 一道横贯
全宽的直线。头条那几行逐条上浮也试过：文章流不参与入场，于是有半秒钟「第一篇是
空的、第二篇却好好地摆在下面」，中间一块两百多像素的白。两样都读作「页面坏了」，
都撤了。装饰可以慢慢到位，读物不能。
